...
import torch
import random
from .target_dataset import TargetClassAwareDataset, TargetUkDataset
...
def get_path_dataset(config, path_dict, length=None, test=False, batch_size=None, get_loader=False):
    ...
    loader = DataLoader(dataset=dataset, batch_size=batch_size, num_workers=config.num_workers, shuffle=not test,
                        drop_last=not test, worker_init_fn=seed_worker)
    if get_loader:
        return loader
    else:
...
def get_dataset(config, dataset, class_set, label_list=None, test=False, batch_size=None, plabel_dict=None,
    ...
    if length is not None and not test:
        ...
        num_steps = None
    if class_wise:
        TargetClassAwareDataset(root_path, config.num_pclass, transforms, class_set, plabel_dict,
                                num_steps=length * config.num_sample)
    else:
        dataset = BaseDataset(root_path, list_path, transforms, dataset, class_set, num_steps=num_steps,
...
def init_pair_dataset(config, src_wei=None, label_list=None, plabel_dict=None, length=None, binary_label=None):
    ...
    tgt_loader = get_dataset(config, config.target, config.target_classes, label_list=label_list,
                             plabel_dict=plabel_dict, length=length, binary_label=binary_label)
...
def init_class_dataset(config, t_plabel_dict, src_class_set, tgt_class_set, length=None, uk_list=None):
    transform = get_transform()
    src_list_path = './dataset/list/{}/{}.txt'.format(config.task, config.source)
    if config.task == 'imagenet-caltech':
    ...
    dataset = ClassAwareDataset(root_path, src_list_path, config.num_pclass, transform, src_class_set, tgt_class_set,
                                t_plabel_dict,
                                num_steps=length * config.num_sample, uk_list=uk_list)
    dataloader = DataLoader(dataset=dataset, batch_size=config.num_sample, num_workers=config.num_workers, shuffle=True,
...
def init_target_dataset(config, plabel_dict, tgt_class_set, length=None, uk_list=None, binary_label=None,
    ...
    if uk_list is not None:
        tgt_class_set.append(config.num_classes)
    dataset = TargetClassAwareDataset(root_path, config.num_pclass, transform, tgt_class_set, plabel_dict, name2index,
                                      num_steps=length * config.num_sample)  # num_steps: 200 * 10
    dataloader = DataLoader(dataset=dataset, batch_size=config.num_sample, num_workers=config.num_workers, shuffle=True,
                            drop_last=True, worker_init_fn=seed_worker)
    dataloader = enumerate(dataloader)
    return dataloader
...
def init_target_uk_dataset(config, plabel_dict, name2index, length):
    transform = get_transform()
    dataset = TargetUkDataset(transform, plabel_dict, name2index, config.uk_index,
                              num_steps=length * config.uk_batch_size)
    dataloader = DataLoader(dataset=dataset, batch_size=config.num_sample, num_workers=config.num_workers, shuffle=True,
                            drop_last=True, worker_init_fn=seed_worker)
    dataloader = enumerate(dataloader)
    return dataloader
